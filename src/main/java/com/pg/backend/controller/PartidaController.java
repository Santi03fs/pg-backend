...
public class PartidaController {
    ...
    private static final String[] PLANTILLAS_PARTIDAS = {
        "Albañilería",
        ...
        for (com.pg.backend.model.Obra obra : obras) {
            ...
            if (list.size() < 30) {
                for (int i = list.size() + 1; i <= 30; i++) {
                    String nombrePartida = (i <= PLANTILLAS_PARTIDAS.length) ? PLANTILLAS_PARTIDAS[i - 1] : ("Partida " + i);
                    Partida p = new Partida(obra.getId(), i, nombrePartida);
    ...
    public List<Partida> obtenerPartidasPorObra(@PathVariable Long idObra) {
        ...
        if (list.size() < 30) {
            // Generar partidas faltantes hasta 30 usando la plantilla
            for (int i = list.size() + 1; i <= 30; i++) {
                String nombrePartida = (i <= PLANTILLAS_PARTIDAS.length) ? PLANTILLAS_PARTIDAS[i - 1] : ("Partida " + i);
                Partida p = new Partida(idObra, i, nombrePartida);
                partidaRepository.save(p);
            }
